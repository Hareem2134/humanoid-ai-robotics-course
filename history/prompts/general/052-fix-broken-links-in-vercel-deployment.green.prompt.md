---
id: 052
title: Fix broken links in Vercel deployment
stage: green
date: 2025-12-17
surface: agent
model: unknown
...
I have fixed the broken links to `/docs/` in the footer and on the homepage. This should resolve the "Docusaurus found broken links!" error on Vercel. Please try deploying again.

## Outcome

- ✅ Impact: Fixed broken links that were causing the Vercel build to fail.
- 🧪 Tests: No tests were run.
- 📁 Files: Modified `frontend/docusaurus-site/docusaurus.config.ts` and `frontend/docusaurus-site/src/pages/index.js` to fix broken links.
- 🔁 Next prompts: The user will try to deploy to Vercel again.
- 🧠 Reflection: The Vercel build process is strict about broken links. The error log was very helpful in identifying the problematic links. The issue was that links to `/docs/` are not pointing to a specific page.
